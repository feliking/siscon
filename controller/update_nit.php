<?php 
	...
	if (!$_FILES['respaldo']['error']==4) {
      ...
      respaldo='$respaldo',
      estado=\"$_POST[estado]\",
      nro_sucursal=\"$_POST[sucursal]\",
      direccion_registro=\"$_POST[direccion]\"
      WHERE id_nit=\"$_POST[ide]\"";
      $query = $con->query($sql);
      if($query!=null){
        print "<script>alert(\"El contrato fue actualizado exitosamente.\");window.location='../views/nit.php';</script>";
      }
      else{
